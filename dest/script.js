...
(function(){
  ...
  (function(){
    var module, FirebaseURL, HeroCtrl, ButtonGroupCtrl, ButtonToolbarCtrl, DelayEvalCtrl;
    module = angular.module;
    FirebaseURL = 'https://angular-on-fire.firebaseio.com';
    HeroCtrl = ['$log', '$scope', 'FireSync'].concat(function($log, $scope, FireSync){
      $scope.counter = new FireSync().get(FirebaseURL + "/counter").sync();
      $log.log('HeroCtrl', $scope);
    });
    ButtonGroupCtrl = ['$log', '$scope', 'FireSync'].concat(function($log, $scope, FireSync){
      $scope.states = new FireSync().get(FirebaseURL + "/button-states").sync();
      $log.log('ButtonGroupCtrl', $scope);
    });
    ButtonToolbarCtrl = ['$log', '$scope', 'FireSync'].concat(function($log, $scope, FireSync){
      $scope.states = new FireSync().get(FirebaseURL + "/button-states", {
        toCollection: true
      }).sync();
      $log.log('ButtonToolbarCtrl', $scope);
    });
    DelayEvalCtrl = ['$log', '$scope', '$timeout', 'FireSync'].concat(function($log, $scope, $timeout, FireSync){
      var stateSync;
      $scope.path = $timeout(function(){
        return 'delayed-eval';
      }, 5000);
      stateSync = new FireSync().get(FirebaseURL + "/{{ path }}/state");
      $scope.state = stateSync.syncWithScope($scope);
      $scope.$on('$destroy', stateSync.destroy);
      $log.log('DelayEvalCtrl', $scope);
    });
    module('demo', ['ui.bootstrap', 'angular-on-fire']).value({
      FirebaseURL: FirebaseURL
    }).controller({
      HeroCtrl: HeroCtrl,
      ButtonGroupCtrl: ButtonGroupCtrl,
      ButtonToolbarCtrl: ButtonToolbarCtrl,
      DelayEvalCtrl: DelayEvalCtrl
    });
  }.call(this));
  (function(){
